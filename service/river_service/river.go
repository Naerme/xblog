...
func NewRiver() (*River, error) {
	...
	var err error
	if r.master, err = loadMasterInfo(global.Conifg.River.DataDir); err != nil {
	...
	cfg.Addr = global.Conifg.ES.Addr
	cfg.User = global.Conifg.ES.Username
	cfg.Password = global.Conifg.ES.Password
	cfg.HTTPS = global.Conifg.ES.IsHttps
...
func (r *River) newCanal() error {
	...
	db := global.Conifg.DB[0]
	cfg.Addr = db.Addr()
	cfg.User = db.User
	cfg.Password = db.Password
	cfg.Charset = "utf8mb4"
	cfg.Flavor = global.Conifg.River.Flavor

	cfg.ServerID = global.Conifg.River.ServerID

	for _, s := range global.Conifg.River.Sources {
...
func (r *River) parseSource() (map[string][]string, error) {
	wildTables := make(map[string][]string, len(global.Conifg.River.Sources))
...
func (r *River) prepareRule() error {
	...
	if global.Conifg.River.Rules != nil {
		// then, set custom mapping rule
		for _, rule := range global.Conifg.River.Rules {
